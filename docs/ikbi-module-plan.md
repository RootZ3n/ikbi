...
| capability-recovery (AMG) | provider (diagnose), injection (untrusted name+evidence+memory), identity, receipt (last-known-good + breakage), events; lab-context-memory + drift-prevention (read-only) — non-executing recovery PLANNER: detects LOST/degraded capabilities, diagnoses cause class (config\|dependency\|registration\|credentials\|model-provider\|path\|permission\|code) → CapabilityRecoveryPlan + recommendedRepair; RECOMMENDS, never invokes (imports no repair module). Distinct from builder (fixes code) + drift (rate decay) |
