...
| Cx | — | **AUTHORITATIVE DECISION WIRED (235565a):** the single-build terminal now uses `decidePromotability` to override the integrator gate when `IKBI_LEGACY_COMPLETION=off` (DEFAULT on = legacy, byte-identical; promote/retain/discard, fail-closed on unavailable facts). Proven both ways (fail-closed + real-git promote-overriding-integrator). **I1/I2/I4/I5/I6/I7 guard fixtures LANDED** (eaa75c8). REMAINING: funnel competitive/tournament/batch/chat-apply/self-heal through the same decision; thread C1c `verifiedAgainst` + re-verify loop; I3/I8/I9 integration guards; then DOGFOOD-validate → flip the default → remove legacy. C1 bugs already fixed (C1a/b/c/C3). | 🟡 core landed |
