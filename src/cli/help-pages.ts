...
export const HELP_PAGES: Readonly<Record<string, HelpPage>> = {
  build: {
    ...
    usage: "ikbi build \"<goal>\" [--repo <path>] [--json] [--yes] [--tier <t>] [--scope[=<path>]] [--max-budget-usd <n>] [--from-pr <n>]",
    flags: [
      { flag: "--repo <path>", desc: "Target repository (defaults to the current directory)." },
      { flag: "--json", desc: "Emit only the JSON result on stdout (logs/hints go to stderr) — the CI-friendly mode. `build` is always non-interactive." },
      ...
      { flag: "--tier <name>", desc: "cheap | mid | frontier — preset builder+critic models per tier." },
      { flag: "--scope[=<path>]", desc: "Staged build from an author-ordered SCOPE.md (bare = SCOPE.md at repo root)." },
      { flag: "--max-budget-usd <n>", desc: "Cap total API spend at $n (budget exhausted = clean abort)." },
    ...
    examples: [
      ...
      { cmd: "ikbi build \"implement the issue\" --json --yes", desc: "CI-friendly invocation (JSON result, no interview)." },
    ],
