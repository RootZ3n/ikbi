...
export const HELP_PAGES: Readonly<Record<string, HelpPage>> = {
  ...
  build: {
    ...
    examples: [
      ...
      { cmd: "ikbi build \"fix the failing auth test\" --repo ../service --strategy shadow", desc: "Two candidates on another repo; the selector promotes one." },
      { cmd: "ikbi build \"implement the issue\" --json", desc: "CI-friendly invocation (full session JSON)." },
    ],
    seeAlso: ["fix", "repl", "legacy", "doctor"],
  },
  legacy: {
    name: "legacy",
    summary: "FROZEN v1 build engine — an emergency fallback during v2 qualification, not the daily driver and not enterable by accident.",
    usage: "ikbi legacy build \"<goal>\" [--repo <path>] [--verbose] [--cost] [--yes] [--no-memory] [--memory-diff]",
    flags: [
      { flag: "build", desc: "The only legacy subcommand — runs the frozen v1 5-role worker pipeline." },
      { flag: "--repo <path>", desc: "Target repository (defaults to the current directory)." },
      { flag: "--yes, -y", desc: "Skip the interactive goal-refinement interview." },
      { flag: "--verbose, -v", desc: "Stream per-role progress events as they fire." },
      { flag: "--cost", desc: "Print a per-role cost breakdown after the build." },
    ],
    examples: [
      { cmd: "ikbi build \"...\"", desc: "Use the GOVERNED v2 engine — the daily driver. Prefer this." },
      { cmd: "ikbi legacy build \"...\" --repo ../service", desc: "Emergency fallback to the frozen v1 engine during qualification only." },
    ],
