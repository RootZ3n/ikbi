 ...
 *   IKBI_SELF_REPAIR_TEST_TIMEOUT_MS hard timeout for the test command, ms. A stuck test
 *                                   command is killed and reported as a test-failure rather
 *                                   than hanging the pass forever. DEFAULT 300000 (5 min).
...
/** Default hard timeout for the test command (5 minutes) — a stuck suite never hangs the pass. */
...
export function loadSelfRepairConfig(reader = env): SelfRepairConfig {
  return Object.freeze({
    ...
    testTimeoutMs: reader.int("TEST_TIMEOUT_MS", DEFAULT_TEST_TIMEOUT_MS, { min: 1000 }),
