...
test("/status reports managed-workspace true, with target repo / workspace / base ref / pending changes", async () => {
  ...
  try {
    ...
    // State-bound candidate writes require a complete observation before replacing an existing file.
    const s = new ChatSession("st-1", { workspace: ws, invoke: queued([toolTurn(
      call("read_file", { path: "README.md" }),
      call("write_file", { path: "README.md", content: "pending\n" }),
    ), stop("done")]) });
    let out = "";
