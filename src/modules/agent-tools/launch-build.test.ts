...
/** A fake spawn that also records the spawn OPTIONS (so a test can inspect the child env). */
function mockSpawnCapturingOpts(stdout: string, code: number, sink: { opts?: { env?: NodeJS.ProcessEnv } }): typeof SpawnType {
  return ((_exec: string, _args: readonly string[], opts: { env?: NodeJS.ProcessEnv }) => {
    sink.opts = opts;
    const child = new EventEmitter() as EventEmitter & { stdout: EventEmitter; stderr: EventEmitter; kill: () => void };
    child.stdout = new EventEmitter();
    child.stderr = new EventEmitter();
...
test("standing instructions are passed to the build via IKBI_BUILD_EXTRA_INSTRUCTIONS (never argv/goal)", async () => {
  const sink: { opts?: { env?: NodeJS.ProcessEnv } } = {};
  const argSink: { args?: readonly string[] } = {};
  const spawnFn = ((exec: string, args: readonly string[], opts: { env?: NodeJS.ProcessEnv }) => {
    argSink.args = args;
    return (mockSpawnCapturingOpts("done\n", 0, sink) as unknown as (e: string, a: readonly string[], o: unknown) => unknown)(exec, args, opts);
  }) as unknown as typeof SpawnType;
  await runLaunchBuild(
    { goal: "add a test" },
    { sessionRepo: "/repo", cliEntry: "cli", execPath: "node", spawnFn, standingInstructions: "  Always use TS strict mode.  " },
  );
  assert.equal(sink.opts?.env?.IKBI_BUILD_EXTRA_INSTRUCTIONS, "Always use TS strict mode.", "trimmed standing instructions ride in the env");
  // The instructions never leak into argv (no injection, no goal pollution).
  assert.ok(!(argSink.args ?? []).some((a) => a.includes("Always use TS strict")), "instructions are NOT in argv");
});

test("no standing instructions ⇒ IKBI_BUILD_EXTRA_INSTRUCTIONS is not set (env unchanged)", async () => {
  const sink: { opts?: { env?: NodeJS.ProcessEnv } } = {};
  const spawnFn = mockSpawnCapturingOpts("done\n", 0, sink);
  await runLaunchBuild({ goal: "add a test" }, { sessionRepo: "/repo", cliEntry: "cli", execPath: "node", spawnFn });
  assert.equal(sink.opts?.env?.IKBI_BUILD_EXTRA_INSTRUCTIONS, undefined, "no extra-instructions env var when none supplied");
});

test("empty/whitespace standing instructions ⇒ no env var set", async () => {
  const sink: { opts?: { env?: NodeJS.ProcessEnv } } = {};
  const spawnFn = mockSpawnCapturingOpts("done\n", 0, sink);
  await runLaunchBuild({ goal: "x" }, { sessionRepo: "/repo", cliEntry: "cli", execPath: "node", spawnFn, standingInstructions: "   " });
  assert.equal(sink.opts?.env?.IKBI_BUILD_EXTRA_INSTRUCTIONS, undefined, "whitespace-only instructions are ignored");
});
